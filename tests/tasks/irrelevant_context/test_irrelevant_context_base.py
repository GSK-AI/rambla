...
from rambla.tasks.irrelevant_context import IrrelevantContextTask

# flake8: noqa: N802


@mock.patch("rambla.tasks.irrelevant_context.irrelevant_context.prepare_dataset")
def test_IrrelevantContextTask_run_task(
    mock_prepare_dataset,
    irrelevant_context_task_config: dict,
    mock_flat_pubmedqa_dataset: Dataset,
    make_mock_llm: Callable,
):
    mock_prepare_dataset.return_value = mock_flat_pubmedqa_dataset
    task = IrrelevantContextTask.from_config(irrelevant_context_task_config)
    responses = [
        "Yes",
        "yes",
        "yes..",
